...
namespace GrafoCoyote.Controllers
{
    ...
    {
        private readonly string[] terrainTypes = new string[5] { "asphalt", "grass", "sand", "water", "pedra" };
        ...
        public Vertex[,] GenerateTerrain(int wid, int hgt, int cellSize, int Ymin, int Xmin)
        {
            ...
            // Inicialize os vizinhos dos nós.
            for (int i = 0; i < hgt; ++i)
            {
                for (int j = 0; j < wid; ++j)
                {
                    if (vertices[i, j].terrainType != "pedra")
                    {
                        ...
                        {
                            int cost;
                            if (vertices[i - 1, j].terrainType == "papaleguas")
                            {
                                cost = 1;
                            }
                            else if (vertices[i - 1, j].terrainType == "coyote")
                            {
                                cost = 1;
                            }
                            else
                            {
                                cost = Array.IndexOf(terrainTypes, vertices[i - 1, j].terrainType);
                            }
                        ...
                        {
                            int cost;
                            if (vertices[i, j - 1].terrainType == "papaleguas")
                            {
                                cost = 1;
                            }
                            else if (vertices[i, j - 1].terrainType == "coyote")
                            {
                                cost = 1;
                            }
                            else
                            {
                                cost = Array.IndexOf(terrainTypes, vertices[i, j - 1].terrainType);
                            }
